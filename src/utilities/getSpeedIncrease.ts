...
const getSpeedIncrease = (statsData: AnalyseSessionResponse) => {
  ...
  const increasePercentages = increasePhases.map((phase) => {
    const previousPhase = statsData.yourPhases[phase - 1];
    const currentPhase = statsData.yourPhases[phase];
...
export default getSpeedIncrease;
